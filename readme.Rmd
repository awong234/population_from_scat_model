...
I have not modeled density, so each of these are outcomes of the random variables *per grid*. There are `r nrow(scaledGrid)` total grid cells, and we sample a total of `r gridsVisited %>% length` cells. The population size among the grid cells we visit in total is as follows:
...
with mean approximately `r ((countSums/popAvail) %>% mean(na.rm = T))*100`%. Notice that the sum of these time-specific available population sizes is NOT the total population size, due to individuals carried between sessions. 
